    ...
    public function onMessage(ConnectionInterface $conn, $msg)
    {
        ...
        if(isset($data->type)){
            if($data->type == 'request_load_unconnected_user'){
                $user_data = User::where('id', '!=', $data->from_user_id)->orderBy('name', 'ASC')->get();
                $sub_data = array();
                foreach($user_data as $row){
                    if(is_null($row->getRawOriginal('profile_photo'))){
                        $user_image = '';
                    }else{
                        $user_image = 'uploads/profile_photo/'.$row->getRawOriginal('profile_photo');
                    }
                    $sub_data[] = array(
                        'name'      =>  $row->name,
                        'id'        =>  $row->id,
            ...
            if($data->type == 'request_search_user'){
                $user_data = User::select('id', 'name', 'user_status', 'profile_photo')
            ...
            {
                $notification_data = Chat_request::where('status', '!=', 'Approve')
                ...
                foreach($notification_data as $row){
                    ...
                    if(is_null($user_data->getRawOriginal('profile_photo'))){
                        $user_image = '';
                    }else{
                        $user_image = 'uploads/profile_photo/'.$user_data->getRawOriginal('profile_photo');
                    }
                    $sub_data[] = array(
                        'id'            =>  $row->id,
            ...
            if($data->type == 'request_connected_chat_user')
            {
                ...
                foreach($user_id_data as $user_id_row){
                    ...
                    $user_data = User::select('id', 'name', 'profile_photo', 'user_status', 'updated_at')->where('id', $user_id)->first();
